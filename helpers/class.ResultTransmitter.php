...
class taoQtiCommon_helpers_ResultTransmitter {
    ...
    public function transmitItemVariable($variables, $transmissionId, $itemUri = '', $testUri = '') {
        ...
        try {
            ...
            $this->getResultService()->getResultServer($executionIdentifier)->getStorageInterface()->storeItemVariables($executionIdentifier, $testUri, $itemUri, $itemVariableSet, $transmissionId);
        }
        catch (Exception $e) {
    ...
    public function transmitTestVariable($variables, $transmissionId, $testUri = '') {
        ...
        try {
            ...
            $executionIdentifier = $this->getSessionFromTransmission($transmissionId);
            $this->getResultService()->getResultServer($executionIdentifier)->getStorageInterface()->storeTestVariables($executionIdentifier, $testUri, $testVariableSet, $transmissionId);
        }
        catch (Exception $e) {
     ...
     * @return string
     */
    private function getSessionFromTransmission($transmission){
        //@TODO replace compatible to \oat\taoQtiTest\models\runner\QtiRunnerService::getTransmissionId
        $x = explode('.', $transmission);
        return count($x) == 3 ? $x[0] : $x[0] . '.' . $x[1];
    }
}
